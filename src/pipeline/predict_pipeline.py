...
class PredictPipeline:
    ...
    def predict(self,features):
        try:
            ...
            domain_encoded = pd.read_excel(domain_encoded_file)
            domain_mapping = dict(zip(domain_encoded['Domaine'], domain_encoded['Domain_encoded']))
            features['Domain'] = features['Domain'].map(domain_mapping)
            logging.info(f" Domaine: {features['Domain']}")
            logging.info(f"features1: {features}")
            features['Gender'] = self.label_encoder.transform(features['Gender'])
            logging.info("encode end success")
            
            features.rename(columns = {'ColonneExperience':'Experience'}, inplace = True) 
            features.rename(columns = {'ColonneNiveau':'Niveau'}, inplace = True)

            column_order=['Niveau', 'Experience', 'Gender', 'Domain']
            features=features[column_order]
            logging.info(f"features2: {features}")
